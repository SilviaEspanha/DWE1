...
function gravar(){
    let result = geraResult();
    let resulXML = '<NOMETUT>' + result[0] + '</NOMETUT>' +
    '<NOMEPET>' + result[1] + '</NOMEPET>' +
    '<DATA>' + result[2] + '</DATA>' +
    '<NOME>' + result[3] + '</NOME>' +
    '<OBS>' + result[4] + '</OBS>' +
    '<PRECO>' + result[5] + '</PRECO>';
    var stringXML = '<COMPROVANTE><SERVICO>' + resulXML + '</SERVICO></COMPROVANTE>';
    var blob = new Blob([stringXML], {type: 'text/xml'});
